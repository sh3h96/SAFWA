...
export default function ReviewsReportsPage() {
  ...
  // Compute real mechanic performance dynamically
  const mechPerformanceMap = {};
  reviews.forEach(r => {
    const mechName = r.mechanic || 'غير محدد';
    if (!mechPerformanceMap[mechName]) {
      mechPerformanceMap[mechName] = { name: mechName, count: 0, sum: 0 };
    }
    mechPerformanceMap[mechName].count += 1;
    mechPerformanceMap[mechName].sum += r.rating;
  });
          ...
          <div className="bg-slate-900 rounded-[2rem] p-8 text-white shadow-xl">
            ...
            <div className="space-y-4">
              {mechanicPerformance.length > 0 ? (
                mechanicPerformance.slice(0, 5).map((mech, idx) => (
                  <div key={idx} className="flex justify-between items-center bg-slate-800/50 p-3 rounded-xl border border-slate-700">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-mono font-bold">{mech.avg}</span>
                      <span className="text-[10px] text-slate-400">({mech.count})</span>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-xs text-slate-400">لا توجد تقييمات للفنيين حالياً</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-2/3 flex flex-col bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
          <div className="p-6 md:p-8 border-b border-slate-50 flex justify-between items-center">
            ...
            <select 
              ...
              className="bg-slate-50 border-none rounded-xl text-xs font-bold text-slate-600 px-4 py-2 outline-none cursor-pointer"
            >
          ...
          <div className="p-6 md:p-8 space-y-6 overflow-y-auto h-[600px] custom-scrollbar">
            {filteredReviews.length > 0 ? (
              ...
              <div className="text-center py-12 text-slate-400">
                ...
                <p className="text-sm">لا توجد تقييمات مطابقة للفترة أو التصفية الحالية</p>
              </div>
            )}
          </div>
        </div>
